...
import { type CampaignMessage } from "../../../../lib/bareplay-types";
...
export async function POST(request: Request) {
  try {
    ...
    if (!campaignName) {
      return NextResponse.json({ error: "Campaign name is required before saving a message." }, { status: 400 });
    }
    if (!name) {
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!campaignName) {
      return NextResponse.json({ error: "Campaign name is required before deleting a message." }, { status: 400 });
    }
    if (!name) {
      return NextResponse.json({ error: "Template name is required." }, { status: 400 });
    }
