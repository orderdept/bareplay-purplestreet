...
const PORT = Number(process.env.BAREPLAY_KEYCHAIN_PORT || 8787);
const SERVICE = "BarePlay Email SMTP";
const DEFAULT_USERNAME = "support@bareplay.org";
const ALLOWED_ORIGINS = new Set([
  "https://bareplay-email.purplestreet.com",
  "https://bareplay-purplestreet.vercel.app",
  "http://localhost:3000",
  "http://127.0.0.1:3000",
]);

function corsHeaders(origin) {
  const allowOrigin = ALLOWED_ORIGINS.has(origin || "") ? origin : "https://bareplay-email.purplestreet.com";
  return {
...
server.listen(PORT, "127.0.0.1", () => {
  console.log(`BarePlay Keychain helper running on http://127.0.0.1:${PORT}`);
