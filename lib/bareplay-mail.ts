...
import { type CampaignDraft, type CampaignMessage } from "./bareplay-types";
...
function personalize(template: string, contact: MailRecipient) {
  const name = contact.name || contact.email.split("@")[0] || "BarePlay Members";
...
<html>
  <body style="font-family: Arial, sans-serif; color: #201725; line-height: 1.45; margin:0; padding:0;">
    ...
    <div style="font-size:12px;color:#63566b;line-height:1.45;">
      <p style="margin:0 0 10px 0;">You are receiving this because you are on the BarePlay customer list.</p>
...
function buildMimeMessage(config: HostedSmtpConfig, message: CampaignMessage, contact: MailRecipient) {
  const boundary = `bareplay-email-${crypto.randomUUID()}`;
  ...
  const messageId = `<${crypto.randomUUID()}@bareplay.org>`;
...
export function hostedSmtpConfigFromDraft(
  ...
  return {
    ...
    username,
    password,
    fromName: draft.fromName.trim() || "BarePlay",
  };
}
