...
const moduleKey = "bareplay-email";
const dataRoot = path.join(process.cwd(), "data", "bareplay");
...
function draftFromCampaign(campaign: CampaignJob | null): CampaignDraft {
  ...
  return {
    ...
    fromName: campaign?.smtp?.fromName || "BarePlay",
...
export async function getBarePlayData() {
  ...
  return {
    moduleKey,
    moduleName: "BarePlay Email",
    businessName: "BarePlay",
    hostname: "bareplay-email.purplestreet.com",
    senderEmail: draft.smtpUsername || latestCampaign?.smtp?.username || "info@bareplay.org",
    senderName: draft.fromName || latestCampaign?.smtp?.fromName || "BarePlay",
    latestCampaign,
    ...
    suppressionDownloads: {
      csv: "/exports/bareplay/suppressed-addresses.csv",
      json: "/exports/bareplay/suppressed-addresses.json",
    },
  };
}
